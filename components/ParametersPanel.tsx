    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <ParamField label="W Tier 2 early penalty" value={params.W_TIER2_EARLY}
            onChange={v => onChange({ W_TIER2_EARLY: number(v, params.W_TIER2_EARLY) })} />
          <Info title="What it does">
            Penalty for scheduling Tier-2 QBank/NIS before all Tier-1 primaries complete. Raises cost for early quesions to delay them behind readings/videos.
          </Info>

          <ParamField label="W Tier 1 quota deficit" value={params.W_TIER1_DEFICIT}
            onChange={v => onChange({ W_TIER1_DEFICIT: number(v, params.W_TIER1_DEFICIT) })} />
          <Info title="What it does">
            Penalty per minute shortfall for Tier-1 weekly study share. Helps front-load primary material over optional.
          </Info>

          <ParamField label="W Physics quota deficit" value={params.W_PHYS_DEFICIT}
            onChange={v => onChange({ W_PHYS_DEFICIT: number(v, params.W_PHYS_DEFICIT) })} />
          <Info title="What it does">
            Penalty per minute shortfall for physics weekly study share.
          </Info>

          <ParamField label="W QBN excess penalty" value={params.W_QBN_EXCESS}
            onChange={v => onChange({ W_QBN_EXCESS: number(v, params.W_QBN_EXCESS) })} />
          <Info title="What it does">
            Penalty per minute excess for QB/NIS weekly ceiling. Helps keep question bank minutes limited early.
          </Info>

          <ParamField label="Workers" value={params.ORTOOLS_WORKERS}
            onChange={v => onChange({ ORTOOLS_WORKERS: number(v, params.ORTOOLS_WORKERS) })} />
          <Info title="What it does">
            Max parallel workers for CP-SAT. 8 is optimum for your rig and plan.
          </Info>

          <ParamField label="Max solve time (seconds)" value={params.ORTOOLS_MAX_TIME}
            onChange={v => onChange({ ORTOOLS_MAX_TIME: number(v, params.ORTOOLS_MAX_TIME) })} />
          <Info title="What it does">
            Max seconds for CP-SAT solver. Use 180 for generation, 60-90 for quick rebalances.
          </Info>
        </div>
      </div>
